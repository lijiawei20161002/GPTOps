...
class CoherenceModel:
    ...
    def calculate_keyword_coverage(self, summary, keywords):
    ...
    def get_coherence(self, text):
        ...
        sentences = [s.strip() for s in text.split('.') if s]
        if not sentences:
            return 0.0  # Return a default value when there are no sentences
        ...
        avg_similarity /= len(sentences)
        return avg_similarity

    def calculate_coherence(self, summary, coherence_model):
        coherence_score = coherence_model.get_coherence(summary)
        return coherence_score

    def calculate_consistency(self, summary):
        ...
        for i in range(len(sentences)):
            for j in range(i + 1, len(sentences)):
                ...
                if self.is_consistent(sentences[i], sentences[j], contradictory_pairs):
    ...
    def is_consistent(self, sentence1, sentence2, contradictory_pairs):
...
def main():
    summary_file_path = 'philly_output_summary.txt'
    original_text_csv_file = 'philly.txt'
    result_file_path = 'evaluation_results.txt'
    keywords = {'GPU', 'utilization', 'machine', 'time', 'ID'}
    ...
    with open(summary_file_path, 'r') as sf, open(original_text_csv_file, 'r', encoding='utf-8') as csvf, open(result_file_path, 'w') as rf:
        ...
        original_texts_per_group = [' '.join([' '.join(map(str, row.values())) for row in group]) for group in groups]

        for i, (summary, original_text) in enumerate(zip(summaries, original_texts_per_group)):
            if len(summary) < 1:
                continue
            #summary = summary.split('\n')[2].strip()  # Extract the summary text
            original_text = original_text.strip()

            rf.write(f'Group {i}:\n')

            compression_ratio = coherence_model.calculate_compression_ratio(original_text, summary)
            keyword_coverage = coherence_model.calculate_keyword_coverage(summary, keywords)
            coherence = coherence_model.calculate_coherence(summary, coherence_model)
            consistency = coherence_model.calculate_consistency(summary)
